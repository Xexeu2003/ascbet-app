...
BTTS_MEDIA_LIGA = { # % MEDIA DE BTTS NA LIGA
...
@st.cache_data(ttl=1800, show_spinner="Buscando Odds REAIS...")
def buscar_jogos_odds(ligas_selecionadas):
    jogos = []; log_erros = []; total_req = 0
    mercados = {"totals": ["Over 1.5", "Over 2.5"], "btts": ["BTTS Sim"]}
    ...
    for liga_id in ligas_selecionadas:
        ...
        r = requests.get(url, params=params); total_req += 1
        if r.status_code == 429: log_erros.append(f"{nome_liga}: ERRO 429"); break
        if r.status_code!= 200: continue
        
        lista_jogos = r.json(); log_erros.append(f"{nome_liga}: {len(lista_jogos)} jogos")
                    ...
                    if market['key'] == 'totals':
                        for outcome in market['outcomes']:
                            if float(outcome['point']) == 1.5 and outcome['name'] == 'Over':
                                odd = outcome['price']; prob_imp = round((1 / odd) * 100, 1); value = prob_15 - prob_imp
                                dt = datetime.fromisoformat(item['commence_time'].replace('Z',''))
                                jogos.append({"Mercado":"Over 1.5FT", "Liga":nome_liga, "Jogo":f"{item['home_team']} x {item['away_team']}", "Data":dt.strftime("%d/%m"), "Hora":dt.strftime("%H:%M"), "Casa":book['title'], "Odd":odd, "Prob Modelo":prob_15, "Prob Casa":prob_imp, "Value %":round(value, 1), "Sinal":"GREEN" if value > 0 else "RED"})
                            if float(outcome['point']) == 2.5 and outcome['name'] == 'Over':
                                odd = outcome['price']; prob_imp = round((1 / odd) * 100, 1); value = prob_25 - prob_imp
                                dt = datetime.fromisoformat(item['commence_time'].replace('Z',''))
                                jogos.append({"Mercado":"Over 2.5FT", "Liga":nome_liga, "Jogo":f"{item['home_team']} x {item['away_team']}", "Data":dt.strftime("%d/%m"), "Hora":dt.strftime("%H:%M"), "Casa":book['title'], "Odd":odd, "Prob Modelo":prob_25, "Prob Casa":prob_imp, "Value %":round(value, 1), "Sinal":"GREEN" if value > 0 else "RED"})
                    
                    if market['key'] == 'btts':
                        for outcome in market['outcomes']:
                            if outcome['name'] == 'Yes':
                                odd = outcome['price']; prob_imp = round((1 / odd) * 100, 1); value = prob_btts - prob_imp
                                dt = datetime.fromisoformat(item['commence_time'].replace('Z',''))
                                jogos.append({"Mercado":"BTTS Sim", "Liga":nome_liga, "Jogo":f"{item['home_team']} x {item['away_team']}", "Data":dt.strftime("%d/%m"), "Hora":dt.strftime("%H:%M"), "Casa":book['title'], "Odd":odd, "Prob Modelo":prob_btts, "Prob Casa":prob_imp, "Value %":round(value, 1), "Sinal":"GREEN" if value > 0 else "RED"})
...
def gerar_pdf(df):
    ...
    c.drawCentredString(width / 2, y, f"RELATORIO ASC.BET V26.16.23"); y -= 20
    c.setFont("Helvetica", 9); c.setFillColor(colors.grey)
    c.drawCentredString(width / 2, y, f"Gerado em: {datetime.now().strftime('%d/%m/%Y %H:%M')} | Fonte: TheOdds API"); y -= 30
    if len(df) == 0: c.drawString(50, y, "Nenhum sinal GREEN encontrado.")
    else:
        for liga in df['Liga'].unique():
            ...
            for mercado in df_liga['Mercado'].unique():
                ...
                c.setFont("Helvetica-BoldOblique", 11); c.setFillColor(colors.HexColor("#D32F2F"))
                c.drawString(35, y, f">>> {mercado} <<<"); y -= 18
                for data_jogo in df_merc['Data'].unique():
                    df_data = df_merc[df_merc['Data'] == data_jogo]
                    c.setFont("Helvetica-Bold", 9); c.setFillColor(colors.HexColor("#424242"))
                    c.drawString(40, y, f"Data: {data_jogo}"); y -= 15
                    ...
                    table.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor("#37474F")), ('TEXTCOLOR', (0,0), (-1,0), colors.whitesmoke), ('ALIGN', (0,0), (-1,-1), 'CENTER'), ('ALIGN', (1,1), (1,-1), 'LEFT'), ('GRID', (0,0), (-1,-1), 0.5, colors.grey), ('FONTNAME', (0,0), (-1,0), 'Helvetica-Bold'), ('FONTSIZE', (0,0), (-1,-1), 7)]))
                    table.wrapOn(c, width, height); table.drawOn(c, 30, y - len(data)*13); y -= len(data)*13 + 10
                    if y < 150: c.showPage(); y = height - 50
            y -= 10
    c.save(); buffer.seek(0); return buffer

# --- INTERFACE ---
st.title("Analisador asc.bet V26.16.23 - THEODDS PRO MAX")
st.success("MERCADOS: Over 1.5, Over 2.5 e BTTS Sim | DADOS 100% REAIS")

col1, col2, col3 = st.columns([2,1,1])
with col1:
    ligas_sel = st.multiselect("1. Escolher Ligas", options=list(LIGAS_ODDS.values()), default=["BRASILEIRÃO SÉRIE A", "MLS - ESTADOS UNIDOS"])
with col2:
    min_value = st.slider("2. Filtro Value Minimo %", -20, 20, -5)
with col3:
    st.write(""); st.write("")
...
if len(jogos_df) > 0:
    ...
    col1.metric("Total de Jogos", len(df)); col2.metric("Sinais GREEN", qtd_green, delta=f"{qtd_green} ops"); col3.metric("Creditos Usados", f"{erros[-1].split(':')[1].strip()}")
    
    def highlight_value(val): return f'color: {"#00C853" if val > 0 else "#D50000"}; font-weight: bold'
...
else: st.warning("Clique em 'BUSCAR DADOS REAIS' para começar.")
